...
function initWidget(waifuPath, apiPath) {
	...
	/* boundle aplayer event */

	ap1.on('play', function () {
		var msg = "开始播放 <span style='color:#0099cc;'>";
		msg = msg + $("#flexaplayer > div.aplayer-body > div.aplayer-info > div.aplayer-music > span.aplayer-title").text();
		msg = msg + "</span>了哦！";
		showMessage(msg, 4000, 9);
	});

	if (typeof wap != "undefined" ? true : false) {
		wap.on('play', function () {
			var msg = "开始播放 <span style='color:#0099cc;'>";
			msg = msg + $("#waplayer > div.aplayer-body > div.aplayer-info > div.aplayer-music > span.aplayer-title").text();
			msg = msg + "</span>了哦！";
			showMessage(msg, 4000, 9);
		});
	}
	...
	function initModel() {
		...
		if (modelId == null) {
			//首次访问加载 指定模型 的 指定材质
			var modelId = 2, //模型 ID
				modelTexturesId = 41; //材质 ID 9 48 7 40 41 24 55
		}
